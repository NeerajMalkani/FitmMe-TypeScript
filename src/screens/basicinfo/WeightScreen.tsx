...
const WeightScreen = ({ theme, selectedWeight }: WeightProp) => {
    ...
    <Animatable.View animation="bounceInUp" duration={1000} delay={10} style={[Styles.flexColumn, Styles.flex6, Styles.paddingVertical8]}>
      <View style={[Styles.flex1, Styles.flexAlignCenter, Styles.flexJustifyCenter]}>
            ...
            <View style={[Styles.flex1, Styles.flexAlignCenter, Styles.flexJustifyCenter, Styles.paddingHorizontal24]}>
              <View style={[Styles.flexRow, Styles.height80]}>
                <View style={[Styles.flex1]}>
                  <ScrollPicker dataSource={CreateNumberArray(30, 240)} selectedIndex={parseInt(selectedWeight[0].toString().split(".")[0]) - 30} renderItem={(data) => NumberPicker(data)} onValueChange={onWeightChange} wrapperHeight={80} wrapperColor={colors.primary} itemHeight={80} highlightColor={multicolors.white} highlightBorderWidth={2} />
                </View>
                <View style={[Styles.flex1, Styles.marginStart16]}>
                  <ScrollPicker dataSource={CreateNumberArray(0, 9)} selectedIndex={parseInt(selectedWeight[0].toString().split(".")[1])} renderItem={(data) => NumberPicker(data)} onValueChange={onWeightChangeDecimal} wrapperHeight={80} wrapperColor={colors.primary} itemHeight={80} highlightColor={multicolors.white} highlightBorderWidth={2} />
                </View>
              </View>
            </View>
          </View>
        ...
        <View style={[Styles.width80, Styles.height80, Styles.flexAlignCenter, Styles.flexJustifyCenter, { position: "absolute", bottom: 24, left: 24, backgroundColor: multicolors.white, elevation: 4, borderRadius: 40 }]}>
          ...
          <Text variant="bodySmall">kgs</Text>
        </View>
      </View>
    </Animatable.View>
  );
